...
def actor_process(actor_id, n_actors, share_dict, device='cuda'):
    actor = Actor(actor_id, n_actors, share_dict, device)
...
class Actor:
    def __init__(self, actor_id, n_actors, shared_dict, device='cpu'):
        ...
        self.net = Q_Base_Net(self.device,action_num=19)
        self.target_net = Q_Base_Net(self.device,action_num=19)
        ...
        self.episode_count = [1] * n_actors
        self.sum_reward = [0] * n_actors
    ...
    def step(self):
        ...
        self.n_steps_memory.add(q_value, state[-self.action_repeat:], h, c, target_h, target_c,
                                action, reward, self.stack_count)
        ...
        if self.n_steps > self.bootstrap_steps:
            ...
            priority = self.calc_priority(pre_q_value, action, reward, q_value, target_q_value,
                                          done)
            self.replay_memory.add(state, h, c, target_h, target_c, action, reward, done,
                                   stack_count, priority)
            self.memory_count += 1
    ...
    def reset(self):
        self.episode_count[self.actor_id] += 1
        self.sum_reward[self.actor_id] += self.episode_reward
        self.average_return = self.sum_reward[self.actor_id] / self.episode_count[self.actor_id]

        self.writer.add_scalar('loss/value/actor{}'.format(self.actor_id), self.average_return, self.episode_count[self.actor_id])
        if self.n_episodes % 1 == 0:
            print('episodes:', self.n_episodes, 'actor_id:', self.actor_id, 'return:',
                  self.episode_reward)
