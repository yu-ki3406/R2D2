...
class ReplayMemory:
    def __init__(self, memory_size=100000, batch_size=32, n_step=3, state_size=(72, 96),
                 cell_size=256, action_repeat=4, n_stacks=4, alpha=0.4):
        self.index = 0
        self.memory_size = memory_size
    ...
    def extend(self, memory):
        ...
        if start_index < last_index:
            ...
            index = [i for i in range(start_index, self.memory_size)] + [i for i in
                                                                         range(last_index)]
    ...
    def update_sequence_priority(self, index, update_pre_next_seq_priority=False):
        for idx in index:
            ...
            self.memory['sequence_priority'][idx] = self.eta * priority.max() + (
            ...
            if update_pre_next_seq_priority:
                ...
                for i in range(1, self.sequence_length + 1):
                    if self.memory['is_seq_start'][(idx + i) % self.memory_size] == 1:
                        ...
                        break
                if update:
                    indices = np.arange(next_idx,
                                        next_idx + self.sequence_length) % self.memory_size
                    priority = self.memory['priority'][indices]
    ...
    def get_stacked_state(self, index):
        ...
        stack_index = [start_index for _ in range(stack_count)] + [
            (start_index + 1 + i) % self.memory_size for i in range(self.n_stacks - stack_count)]
    ...
    def sample(self, device='cuda'):
        ...
        batch = dict()
        batch['state'] = [
            np.stack([self.get_stacked_state(i % self.memory_size) for i in seq_index + s]) for s in
            range(self.sequence_length)]
        batch['next_state'] = [
            np.stack([self.get_stacked_state(i % self.memory_size) for i in next_seq_index + s]) for
        ...
        batch['action'] = [
            self.memory['action'][(seq_index + self.burn_in_length + s) % self.memory_size] for s in
            range(self.learning_length)]
        batch['reward'] = [
            self.memory['reward'][(seq_index + self.burn_in_length + s) % self.memory_size] for s in
            range(self.learning_length)]
        batch['done'] = [
            self.memory['done'][(seq_index + self.burn_in_length + s) % self.memory_size] for s in
            range(self.learning_length)]
    ...
    def indexing_sample(self, start_index, last_index, device='cpu'):
        ...
        batch['next_state'] = np.stack(
            [[self.get_stacked_state(i % self.memory_size)] for i in next_index])
